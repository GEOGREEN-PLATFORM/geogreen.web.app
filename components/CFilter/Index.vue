  ...
  <div class="c-filter-container">
    <div class="filter-button">
      <GGButton
        ...
        text-color="var(--app-grey-500)"
        @click="toggleContentShow"
      >
        <div class="filter-button__content">
          <q-icon :name="mdiFilterOutline" color="blue-500" />
    ...
    <Transition name="slide-fade">
      <div v-show="showContent" class="filter-content">
        <div class="filter-items">
          <div v-for="item in filterItems" :key="item.key" class="filter-items__item">
            <div v-if="item.type === 'select' && item.data" class="filter-items__select">
              <KTInputSelect
            ...
            <div v-if="item.type === 'date-range'" class="filter-items__date-range">
              <KTInputDate v-model="item.selected[0]" :label="item.label + ' с'" />
              <span class="filter-items__date-range-divider" />
              <KTInputDate v-model="item.selected[1]" :label="item.label + ' по'" />
            </div>
          </div>
        </div>
        <div class="filter-content__footer">
          <div class="filter-content__btn filter-content__btn--apply" @click="updateValue">
            Применить
          </div>
          <div class="filter-content__btn filter-content__btn--reset" @click="resetFilters">
            Сбросить
          </div>
        </div>
      </div>
